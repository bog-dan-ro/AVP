...
void InitAVPNetGame(void)
{
    ...
    if(AvP.Network==I_Host)
    {
        ...
        strncpy(netGameData.playerData[0].name, AVPDPplayerName, NET_PLAYERNAMELENGTH-1);
...
/*----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------*/
void ProcessGameMessage(DPID senderId, const char * msgP,unsigned int msgSize)
{
    char* subMessagePtr;
        ...
        for(i=0;i<NET_MAXPLAYERS;i++)
        {
            ...
            if(netGameData.playerData[i].playerId != messagePtr->players[i].playerId)
            {
                if(messagePtr->players[i].playerId)
                {
                    ...
                    hr=NetGetPlayerName(glpDP,messagePtr->players[i].playerId, 0, &size);
                    if(hr==DP_OK || hr==DPERR_BUFFERTOOSMALL)
                    {
                        //allocate buffer to recive the name
                        data=AllocateMem(size);
                        *data=0;
                        hr=NetGetPlayerName(glpDP,messagePtr->players[i].playerId, data, &size);
                        if(hr==DP_OK)
                        {
                            strncpy(netGameData.playerData[i].name, data, NET_PLAYERNAMELENGTH-1);
